<#
.SYNOPSIS
    Builda i binari Speedy e produce l'installer Windows (.exe) con Inno Setup.

.DESCRIPTION
    Sequenza completa:
      1. Legge la versione da Cargo.toml (workspace root)
      2. Esegue build-release.ps1 → dist\*.exe
...
# ------------------------------------------------------------------
Write-Host '==> [3/3] Ricerca iscc.exe...' -ForegroundColor Yellow
...
# ------------------------------------------------------------------
Write-Host ''
Write-Host "    Compilazione installer v$version..." -ForegroundColor DarkGray
...
if (Test-Path $outFile) {
    ...
    Write-Host "Attenzione: file atteso non trovato: $outFile" -ForegroundColor Yellow
